...
const addOrder = async (
    ...
    const productIds = validateData.cart.map(item => new mongoose.Types.ObjectId(item.id));
    ...
    for (const item of validateData.cart) {
        ...
        if (item.quantity > productQuantities[item.id]) {
            throw new AppError(400, `${products
                .find((product) => product._id === new mongoose.Types.ObjectId(item.id))?.title
